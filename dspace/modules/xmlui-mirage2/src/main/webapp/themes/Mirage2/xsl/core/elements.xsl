...
-->
    ...
    <xsl:template match="dri:xref">    
	    <!--    <<xsl:if test="not(contains(@target, 'personft'))"> -->
        <a>
            <xsl:if test="@target">
    ...
    <xsl:template match="dri:div[@interactive='yes']" priority="2">
        ...
        <xsl:if test="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='page'][@qualifier='jumpTo']">
         ...
         <xsl:if test="@id='aspect.artifactbrowser.ConfigurableBrowse.div.browse-navigation' and contains(//dri:metadata[@qualifier='queryString'], 'type=person')">
		<span class="filter"><i18n:text>xmlui.ArtifactBrowser.Navigation.browse_ft.label</i18n:text>:
		<span>
			<a class="ft" href="/browse?type=personftadvisor"><i18n:text>xmlui.ArtifactBrowser.Navigation.browse_personftadvisor</i18n:text></a>
		</span>
		|
		<span>
			<a class="ft" href="/browse?type=personftreferee"><i18n:text>xmlui.ArtifactBrowser.Navigation.browse_personftreferee</i18n:text></a>
		</span>
		|
		<span>
			<a href="/browse?type=person"><i18n:text>xmlui.ArtifactBrowser.Navigation.browse_person</i18n:text></a>
		</span>
		</span>
        </xsl:if>
        <xsl:apply-templates select="@pagination">
