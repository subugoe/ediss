...
<xsl:stylesheet
	...
	<xsl:variable name="handle">
        ...
        <div>
		    ...
		    <div rend="col-xs-10 col-sm-{12 - $left-width}">
			    <xsl:choose>
				    <xsl:when test="(//dri:metadata[@element='request' and @qualifier='URI'] = 'publish-now')">
					    <xsl:apply-templates select="document($externalMetadataURL)" mode="community-browser-unlink"/>
				   </xsl:when>
				   <xsl:otherwise>
				...
				<xsl:when test="(//dri:metadata[@element='request' and @qualifier='URI'] = 'publish-now')">
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:apply-templates select="document($externalMetadataURL)" mode="community-browser-submit"/>
						</xsl:otherwise>
					</xsl:choose>
			    </xsl:when>
                            <xsl:otherwise>
    ...
    <xsl:template match="mets:METS" mode="community-browser-submit">
        <xsl:variable name="dim" select="mets:dmdSec/mets:mdWrap/mets:xmlData/dim:dim"/>
	<xref target="{concat(@OBJID, '/submit')}" n="community-browser-link">
            <xsl:value-of select="$dim/dim:field[@element='title']"/>
	</xref> 
        <!--Display community strengths (item counts) if they exist-->
	<!-- <xsl:if test="string-length($dim/dim:field[@element='format'][@qualifier='extent'][1]) &gt; 0">
    ...
    <xsl:template match="mets:METS" mode="community-browser-unlink">
        <xsl:variable name="dim" select="mets:dmdSec/mets:mdWrap/mets:xmlData/dim:dim"/>
	<span><span>
            <xsl:value-of select="$dim/dim:field[@element='title']"/>
    </span></span>
